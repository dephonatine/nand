...
int does_sym_exist(uint8_t *token) {
    struct symbol_data *s = symhead;
    uint8_t *tmp = token;
    while(s->list.next) { 
        if(!strcmp(token, s->symbol)) {
            printf("ALREADY FOUND SYMBOL %s\n", tmp);
            return 1;
        }
        s = s->list.next;
    }
...
void first_pass(struct line_data *ptr) { 
    ...
    uint8_t *tok;
    uint8_t *tok2;
    /* Handle function symbols first because their address is position dependent */
    while(ptr) { 
        if(strstr(ptr->data, ")")) {
            tok = strtok(ptr->data, ")");
            symcur->symbol = ++tok;
...
next:
    ...
    while(ptr) {        
    if(strstr(ptr->data, "@")) { 
            if (ptr->data[1] < 0x3A || ptr->state == DONE)
                goto next2;
            tok2 = strtok(ptr->data, "@");
            strtok(ptr->data, "/");
            strtok(ptr->data, " ");
